            ...
            if cnt >= self._min_support:
                ...
                for g_key in self.graphs:
                    if self.graphs[g_key].gid in vevlb_dict[vevlb]:
                        copy_list_freq = copy.deepcopy(list_freq)
                        list_freq = []

                        for pair in vevlb_dict[vevlb][self.graphs[g_key].gid]:
                            self.graphs[g_key].set_freq_edge(pair[0], pair[1])
                            if not copy_list_freq:
                                list_freq.append({self.graphs[g_key].gid: pair})
                            else:
                                for appear in copy_list_freq:
                                    appear.update({self.graphs[g_key].gid: pair})
                                    list_freq.append(appear)

                # Append into set
                for freq in list_freq:
                    new_tree = SG_container(g, freq)
                    self._frequent_size1_subgraphs.append(new_tree)
                    __freq_edge.append(SG_container(g, freq, forFreqEdge=True))
